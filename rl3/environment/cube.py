...
"""
This file was adapted from the Magic Cube project: https://github.com/davidwhogg/MagicCube
...
class Cube(object):
    ...
    def __init__(self, N, seed=123, whiteplastic=False):
        """
        (see above)
        """
        self.N = N
    ...
    def is_solved(self):
        return all([len(set(s_face.flatten())) == 1 for s_face in self.stickers])

    def get_state(self):
        return self.stickers.flatten()
...
if __name__ == "__main__":
    ...
    c = Cube(3, seed=seed, whiteplastic=False)
#    c.turn("U", 1)
#    c.move("U", 0, -1)
...
#    checkerboard(c)
    for m in range(10):
        print "State: %s" % str(c.get_state())
        print "It's solved!" if c.is_solved() else "Not solved!"
